...
export const INITIAL_STUDENTS = [
  ...
  },
  {
    ...
    mockSessions: [
      ...
      { id: 'mock-212-4', date: '2026-07-29', score: 9.5, evaluator: 'Piyas', category: 'Practical', feedback: 'Flawless database sharding and caching strategy. Outstanding performer ready for tech rounds.', strengths: 'Distributed systems & sharding', improvement: 'Polishing verbal delivery', transcript: 'Interviewer (Piyas): Welcome Avirup! Today we will evaluate your practical system design skills on database sharding and multi-region caching. Let’s start with your approach for handling 100,000 write requests per second.\n\nCandidate (Avirup): Thank you Piyas. For 100k writes/sec, relying on a single database node is impossible due to I/O bottlenecks. I would design a horizontal database sharding layer using consistent hashing with virtual nodes to evenly distribute write keys across 8 primary PostgreSQL shards.\n\nInterviewer (Piyas): Excellent. How do you handle cross-shard queries or join operations?\n\nCandidate (Avirup): Great question. Cross-shard joins introduce massive network latency. I prevent cross-shard joins by denormalizing read-heavy entities and using a Redis Enterprise cluster as a write-through cache layer. Secondary indexes are indexed asynchronously via Kafka consumer groups.\n\nInterviewer (Piyas): Outstanding explanation! What happens if a shard primary fails?\n\nCandidate (Avirup): We use automated failover with Raft consensus via Patroni. The standby replica is promoted in under 3 seconds while client connections retry gracefully using exponential backoff.' }
    ],
    stickyNotes: [
      {
        id: 'note-212-1',
